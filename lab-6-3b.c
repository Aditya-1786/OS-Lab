...
key1 = 0x10081;
 ...
 if ((shm1 = shmat(shmid1, NULL, 0)) == (char *) -1)// attached to memory addressprovided by system
...
printf("\nBefore turn %d, s0= %d s1= %d %s in CR\n",*turn,*s0,*s1,argv[0]);
 //check
 *s1=0; //P2 want to enter
 while(*s0==0) // P1 wants and in CR
{if(*turn==1) // P1 turn
 { *s1=1; //P2 give up chance
 while(*turn==1);// wait until turn change to 2
 *s1=0;
 } }
 ;//wait for turn to become 2
 printf("\n\t\tIn CS turn= %d, s0= %d s1= %d ,%s in CR\n",*turn,*s0,*s1,argv[0]);
 sleep(rand()%5);//sleep for random duration 0 to 4 sec
//release
 sleep(2);
 *turn=1; //now P1 may enter
 *s1=1;// P2 outside CR
